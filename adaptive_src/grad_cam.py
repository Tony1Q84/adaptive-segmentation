...
def modify_backprop(model, name, task):
    ...
    with graph.gradient_override_map({'Relu':name}):
        ...
        if task == 'gender':
            ...
            model_path = '../trained_models/emotion_models/fer2013/fer2013_mini_XCEPTION.102-0.66.hdf5'
...
def compile_gradient_function(input_model, category_index, layer_name):
    ...
    gradient_function = K.function([model.layers[0].get_input_at(node_index=0), K.learning_phase()],
                                   [conv_output, gradients])
...
if __name__ == '__main__':
    ...
    preprocess_input = load_model(face)
    ...
    register_gradient()
    guided_model = modify_backprop(model, 'GuidedBachProp')
    saliency_function = compile_saliency_function(guided_model)
    guided_gradCAM = calculate_guided_gradient_CAM(preprocess_input,
